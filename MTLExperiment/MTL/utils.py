...
def get_metric_and_best_threshold(y_test,y_pred,show_result=True,device='cpu',metric='f1'):
    ...
    return result
########################### final stat ##########################
def Final_metrics(y_pred,y_test_metrix,y_test_CN,best_threshold,DataSetName,epoch):
    ...
    for x in y_pred.keys():
        ...
        result['treshold'][x] = best_threshold[x]['thresholds']
        result['EarlyStopping'][x] = epoch
