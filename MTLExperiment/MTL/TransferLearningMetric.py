...
def task_affitity(net,trainingset,testset,task_list,loss_fn,learning_rate,weight_decay,step_size,gamma,device):
    ...
    for tsk_set in tqdm(all_task_subsets):
        ...
        if task_subset_i != tsk_set[0]:
            ...
            task_layers_list_i = []
            for task_name,task in model.task_specific_models.named_children():
                if task_name == task_subset_i:
            ...
            temp_model = SubModel(shared_layers=model.shared_layers) 
            # use deep copy to ensure to not update the original model
            ...
            optimizer_i = optim.Adagrad(temp_model_i.parameters(),lr=learning_rate,weight_decay=weight_decay)

            optimizer_sh = optim.Adagrad(temp_model_sh.parameters(),lr=learning_rate,weight_decay=weight_decay)
            ...
            for batch_idx, (data) in enumerate(trainingset):
                ...
                output_i = {}
                for task_model in temp_model_i.task_layers_list:
                    output_i[task_model.__class__.__name__.replace('Network','')] = task_model(out_sh)

                ##### loss with CN #####
                combined_loss = []
                for task, output in output_i.items():
                    loss_task = F.binary_cross_entropy(torch.squeeze(output,dim=1),data[task].float())
                ...
                with torch.no_grad():
                    ...
                    output_i_new = {}
                    for task_model in temp_model_i.task_layers_list:
                        output_i_new[task_model.__class__.__name__.replace('Network','')] = task_model(out_sh)
        ...
        with torch.no_grad():
            for batch_idx, (data) in enumerate(testset):
                ...
                output_j_t1 = {}
                for task_model in temp_model_j.task_layers_list:
                    output_j_t1[task_model.__class__.__name__.replace('Network','')] = task_model(out_sh_t1)
                ...
                output_j_t0 = {}
                for task_model in temp_model_j.task_layers_list:
                    output_j_t0[task_model.__class__.__name__.replace('Network','')] = task_model(out_sh_t0)
                
                combined_loss = []
                for task, output in output_j_t0.items():
                    loss_task = F.binary_cross_entropy(torch.squeeze(output,dim=1),data[task].float())
                    combined_loss.append(loss_task)
